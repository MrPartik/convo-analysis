...
@elseif(@$type === 'action')
    <div class="flex space-x-1 justify-around">
        ...
        @if(intval($id) !== \Illuminate\Support\Facades\Auth::id())
            ...
            @else
                <button title="Activate User" wire:click="toggleVisibility({{$id}})" class="flex items-center space-x-2 px-3 border border-green-400 rounded-md bg-white text-green-400 text-xs leading-4 font-medium uppercase tracking-wider hover:bg-green-100 focus:outline-none">
                    <svg class="h-4 w-5 stroke-current m-2" fill="currentColor" viewBox="0 0 50 50" xmlns="http://www.w3.org/2000/svg">
